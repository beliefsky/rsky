...
impl Socket {
    pub(crate) fn new<T>(domain: &T, socket_type: i32) -> io::Result<Self> where T: SocketDomain {
        { // init
        ...
        let success = unsafe {
            WinSock::ioctlsocket(fd, WinSock::FIONBIO, &mut 1)
        } == 0;
    ...
    pub fn bind<T>(&self, addr: &T) -> io::Result<()> where T: SocketAddrNative {
        ...
        let success = unsafe {
            WinSock::bind(self.fd, natvie_addr.as_ptr(), addr_len)
        } == 0;
    ...
    pub fn listen(&self, backlog: u32) -> io::Result<()> {
        ...
        let success = unsafe {
            WinSock::listen(self.fd, backlog)
        } == 0;
...
impl SocketAddrNative for SocketAddrV6 {
    ...
    fn native_addr(&self) -> (SocketAddrCRepr, i32) {
        ...
        (sockaddr, std::mem::size_of::<WinSock::SOCKADDR_IN6>() as i32)
    }
}
